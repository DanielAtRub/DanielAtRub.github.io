document.addEventListener('DOMContentLoaded', () => {
        ...
        for (const prefix of candidatePrefixes) {
            ...
            for (const ext of candidateExts) {
                ...
                for (const pad of candidatePads) {
                    ...
                    const url =
                        `${prefix}${String(1).padStart(pad, '0')}.${ext}`;
    ...
    (async function initBackground() {
        ...
        const makeUrl =
            i =>
                `${prefix}${String(i).padStart(pad, '0')}.${ext}`;
        ...
        async function downloadFrame(idx) {
            ...
            try {
                ...
                const img =
                    await loadImagePromise(makeUrl(idx));
                ...
                // Ignore individual frame loading errors
            }
        }
        ...
        window.__updateAnimatedBackground =
            updateByPercent;
    ...
    function onScroll() {
        ...
        requestAnimationFrame(() => {
            ...
            if (
                typeof window.__updateAnimatedBackground ===
                'function'
            ) {
    ...
    /* ACHIEVEMENTS                                          */
    /* ===================================================== */
    const achievementToastEl =
        safeGet('achievement-toast');
    ...
    function showAchievementToast(keyName) {
        ...
        const msgPrefix =
            translations[currentLang].ach_unlocked;
        ...
        if (achievementToastEl) {
            ...
            achievementToastEl.classList.add(
                'show'
            );

            setTimeout(() => {

                achievementToastEl.classList.remove(
                    'show'
                );
    ...
    function checkAchievements(pct) {
        ...
        if (
            pct >=
            achievementList.inicio.threshold
        ) {
        ...
        if (
            pct >=
            achievementList.mitad.threshold
        ) {
            unlockAchievement('mitad');
        }

        if (
            pct >=
            achievementList.final.threshold
        ) {
            unlockAchievement('final');
        }
    }
});